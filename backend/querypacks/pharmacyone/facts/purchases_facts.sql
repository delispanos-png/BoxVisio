SELECT
  ...
  CAST(
    (CASE WHEN ISNULL(F.TFPRMS, 0) IN (151, 152) THEN -1 ELSE 1 END)
    * COALESCE(TRY_CAST(ABS(ISNULL(L.LINEVAL, ISNULL(L.NETLINEVAL, 0))) AS decimal(28,8)), 0)
    AS decimal(28,8)
  ) AS line_value,
  CAST(
    (CASE WHEN ISNULL(F.TFPRMS, 0) IN (151, 152) THEN -1 ELSE 1 END)
  ...
  CAST(
    (CASE WHEN ISNULL(F.TFPRMS, 0) IN (151, 152) THEN -1 ELSE 1 END)
    * COALESCE(TRY_CAST(ABS(ISNULL(L.PRICE, 0)) * ABS(ISNULL(L.QTY1, ISNULL(L.QTY, 0))) AS decimal(28,8)), 0)
    AS decimal(28,8)
  ) AS no_discount_amount,
  ...
  CAST(ISNULL(S.AFM, '') AS nvarchar(64)) AS supplier_afm,
