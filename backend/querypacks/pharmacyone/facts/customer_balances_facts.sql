-- Customer balances, one row per customer and company, as of the requested date (today when unset).
-- Ids carry the CB2| prefix: readers use only ledger rows once a tenant has them.
--
-- The balance comes from SoftOne's own trader ledger (TRDTRN), signed by the
-- movement type (TPRMS.FLG01 debit / FLG02 credit): debit minus credit is what
-- the customer owes. This reproduces the customer card; the old version summed
-- every FINDOC.SUMAMNT since 2016 with no opening balance (trillions from corrupt rows).
...
--
-- Incremental runs return only the customers with a document changed since the
-- sync cursor; a full run (no cursor) returns every customer with a non-zero balance.
SELECT * FROM (
SELECT
  CAST(ISNULL(T.CODE, CAST(B.TRDR AS nvarchar(64))) AS nvarchar(64)) AS customer_id,
  ...
  CAST(LP.TRNDATE AS date) AS last_collection_date,
  CAST(0 AS decimal(18,4)) AS trend_vs_previous,
  CAST('EUR' AS nvarchar(3)) AS currency,
  CAST('CB2|' + ISNULL(T.CODE, CAST(B.TRDR AS nvarchar(64))) + '|' + CAST(B.COMPANY AS nvarchar(32)) + '|' + CONVERT(varchar(10), COALESCE(CAST(@to_date AS date), CAST(GETDATE() AS date)), 23) AS nvarchar(128)) AS external_id,
  CAST(B.updated_at AS datetime2) AS updated_at,

  CAST(ISNULL(T.CODE, CAST(B.TRDR AS nvarchar(64))) AS nvarchar(64)) AS customer_ext_id,
...
FROM (
  ...
  INNER JOIN TRDR TK WITH (NOLOCK) ON TK.TRDR = K.TRDR AND TK.COMPANY = K.COMPANY AND TK.SODTYPE = 13
...
OUTER APPLY (
  ...
  WHERE FP.COMPANY = B.COMPANY AND FP.TRDR = B.TRDR AND FP.SOSOURCE IN (1381, 1413)
...
OUTER APPLY (
  ...
  FROM (
    ...
    FROM (
      SELECT
        X.TRNDATE,
        X.LTRNVAL * (P.FLG01 - P.FLG02) AS amount,
        SUM(X.LTRNVAL * (P.FLG01 - P.FLG02)) OVER (ORDER BY X.TRNDATE DESC, X.FINDOC DESC, X.TRDTRN DESC ROWS UNBOUNDED PRECEDING) AS running
      FROM TRDTRN X WITH (NOLOCK)
      INNER JOIN TPRMS P WITH (NOLOCK) ON P.COMPANY = X.COMPANY AND P.SODTYPE = X.SODTYPE AND P.TPRMS = X.TPRMS
      WHERE X.COMPANY = B.COMPANY
        AND X.TRDR = B.TRDR
        AND ISNULL(BAL.open_balance, 0) > 0
        AND X.LTRNVAL * (P.FLG01 - P.FLG02) > 0
        AND ABS(ISNULL(X.LTRNVAL, 0)) < 1000000000
