SELECT
  ...
  CAST(SUM(
    CASE
      ...
      WHEN ISNULL(F.SOSOURCE, 0) <> 1351 AND ISNULL(F.TFPRMS, 0) IN (101, 131) THEN ABS(COALESCE(TRY_CONVERT(decimal(28,6), F.SUMAMNT), 0))
      WHEN ISNULL(F.SOSOURCE, 0) <> 1351 AND ISNULL(F.TFPRMS, 0) IN (102, 181) THEN -ABS(COALESCE(TRY_CONVERT(decimal(28,6), F.SUMAMNT), 0))
...
WHERE
  ...
  AND (
    ...
    OR (ISNULL(F.SOSOURCE, 0) <> 1351 AND ISNULL(F.TFPRMS, 0) IN (101, 102, 131, 181))
