SELECT
  ...
  CAST(SUM(
    CASE
      WHEN F.SOSOURCE IN (1251, 1261, 1653) THEN ABS(ISNULL(F.SUMAMNT, 0))
      WHEN F.SOSOURCE IN (1253) THEN -ABS(ISNULL(F.SUMAMNT, 0))
...
WHERE
  ...
  AND
  F.SODTYPE = 12
  AND F.SOSOURCE IN (1251, 1253, 1261, 1281, 1412, 1416, 1653)
