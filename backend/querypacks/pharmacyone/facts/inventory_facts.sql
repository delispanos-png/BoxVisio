SELECT
  ...
  COALESCE(TRY_CAST(RBL.REMAINLIMMIN AS decimal(18,4)), CAST(1 AS decimal(18,4))) AS min_stock,
  COALESCE(TRY_CAST(RBL.REORDERLEVEL AS decimal(18,4)), CAST(1 AS decimal(18,4))) AS replenishment_moq,
...
LEFT JOIN MTRBRNLIMITS RBL WITH (NOLOCK)
  ON RBL.MTRL = I.MTRL
 AND RBL.COMPANY = I.COMPANY
 AND RBL.BRANCH = ISNULL(BR.BRANCH, ISNULL(NULLIF(W.WHOUSEG, 0), ISNULL(S.WHOUSE, 0)))
...
LEFT JOIN MTRBRNLIMITS RBL WITH (NOLOCK) ON RBL.MTRL = I.MTRL AND RBL.COMPANY = I.COMPANY AND RBL.BRANCH = F.BRANCH
