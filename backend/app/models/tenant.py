...
class DimItem(TenantBase):
    ...
    alternate_barcodes: Mapped[str | None] = mapped_column(Text, nullable=True)
    softone_sotype: Mapped[int | None] = mapped_column(Integer, nullable=True, index=True)
    ...
    manufacturer_code: Mapped[str | None] = mapped_column(String(128), nullable=True, index=True)
...
class FactSales(TenantBase):
    ...
    channel_ext_id: Mapped[str | None] = mapped_column(String(64), nullable=True)
    ...
    profit_amount: Mapped[float] = mapped_column(Numeric(18, 2), default=0)
...
class FactSalesDocumentCharge(TenantBase):
    ...
    amount_tax: Mapped[float] = mapped_column(Numeric(18, 2), nullable=False, default=0)
    amount_gross: Mapped[float] = mapped_column(Numeric(18, 2), nullable=False, default=0)
    source_connector_id: Mapped[str | None] = mapped_column(String(64), nullable=True, index=True)
    source_payload_json: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False, index=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)
...
class FactPurchases(TenantBase):
    ...
    discount1_amount: Mapped[float | None] = mapped_column(Numeric(18, 2), nullable=True)
    discount2_amount: Mapped[float | None] = mapped_column(Numeric(18, 2), nullable=True)
    discount3_amount: Mapped[float | None] = mapped_column(Numeric(18, 2), nullable=True)
    discount_pct: Mapped[float | None] = mapped_column(Numeric(10, 4), nullable=True)
    discount_amount: Mapped[float | None] = mapped_column(Numeric(18, 2), nullable=True)
    net_value: Mapped[float] = mapped_column(Numeric(18, 2), default=0)
    cost_amount: Mapped[float] = mapped_column(Numeric(18, 2), default=0)
...
class FactInventory(TenantBase):
    ...
    cost_amount: Mapped[float] = mapped_column(Numeric(18, 2), default=0)
    value_amount: Mapped[float] = mapped_column(Numeric(18, 2), default=0)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False, index=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)
...
class FactSupplierBalance(TenantBase):
    ...
    branch_ext_id: Mapped[str | None] = mapped_column(String(64), nullable=True, index=True)
    source_connector_id: Mapped[str | None] = mapped_column(String(64), nullable=True, index=True)
    open_balance: Mapped[float] = mapped_column(Numeric(18, 2), default=0)
    overdue_balance: Mapped[float] = mapped_column(Numeric(18, 2), default=0)
    aging_bucket_0_30: Mapped[float] = mapped_column(Numeric(18, 2), default=0)
    aging_bucket_31_60: Mapped[float] = mapped_column(Numeric(18, 2), default=0)
    aging_bucket_61_90: Mapped[float] = mapped_column(Numeric(18, 2), default=0)
    aging_bucket_90_plus: Mapped[float] = mapped_column(Numeric(18, 2), default=0)
    last_payment_date: Mapped[date | None] = mapped_column(Date, nullable=True)
    trend_vs_previous: Mapped[float | None] = mapped_column(Numeric(18, 2), nullable=True)
    currency: Mapped[str] = mapped_column(String(3), nullable=False, default='EUR')
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False, index=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)
...
class FactSupplierOrder(TenantBase):
    ...
    line_value: Mapped[float] = mapped_column(Numeric(18, 2), default=0)
...
class AggCashDaily(TenantBase):
    ...
    inflows: Mapped[float] = mapped_column(Numeric(18, 2), default=0)
    outflows: Mapped[float] = mapped_column(Numeric(18, 2), default=0)
...
class AggSupplierBalancesDaily(TenantBase):
    ...
    aging_bucket_61_90: Mapped[float] = mapped_column(Numeric(18, 2), default=0)
    aging_bucket_90_plus: Mapped[float] = mapped_column(Numeric(18, 2), default=0)
    trend_vs_previous: Mapped[float] = mapped_column(Numeric(18, 2), default=0)
...
class AggStockAging(TenantBase):
    ...
    branch_ext_id: Mapped[str | None] = mapped_column(String(64), nullable=True, index=True)
    qty_on_hand: Mapped[float] = mapped_column(Numeric(18, 4), default=0)
    stock_value: Mapped[float] = mapped_column(Numeric(18, 2), default=0)
...
class ReplenishmentLine(TenantBase):
    ...
    vendor_moq: Mapped[float | None] = mapped_column(Numeric(18, 4), nullable=True)
    purchase_price: Mapped[float | None] = mapped_column(Numeric(18, 4), nullable=True)
    ...
    supplier_order_value: Mapped[float] = mapped_column(Numeric(18, 2), nullable=False, default=0)
...
class EraExplorationSnapshot(TenantBase):
    __tablename__ = 'era_exploration_snapshots'
    __table_args__ = (
        Index('ix_era_exploration_snapshots_imported_at', 'imported_at'),
        Index('ix_era_exploration_snapshots_period', 'period_label'),
        Index('ix_era_exploration_snapshots_source_sha256', 'source_sha256'),
    )
...
class EraExplorationLine(TenantBase):
    __tablename__ = 'era_exploration_lines'
    __table_args__ = (
        Index('ix_era_exploration_lines_snapshot_brand', 'snapshot_id', 'brand'),
        Index('ix_era_exploration_lines_snapshot_category', 'snapshot_id', 'category'),
        Index('ix_era_exploration_lines_primary_barcode', 'primary_barcode'),
        Index('ix_era_exploration_lines_market_sales', 'snapshot_id', 'market_sales'),
    )

    id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), primary_key=True, server_default=text('gen_random_uuid()'))
    snapshot_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), ForeignKey('era_exploration_snapshots.id', ondelete='CASCADE'), nullable=False)
    source_row: Mapped[int] = mapped_column(Integer, nullable=False)
    ...
    barcodes_json: Mapped[list] = mapped_column(JSONB, nullable=False, default=list)
    category: Mapped[str | None] = mapped_column(String(255), nullable=True)
    market_sales: Mapped[float] = mapped_column(Numeric(18, 4), nullable=False, default=0)
    your_sales: Mapped[float] = mapped_column(Numeric(18, 4), nullable=False, default=0)
    your_sales_value_ms: Mapped[float] = mapped_column(Numeric(18, 4), nullable=False, default=0)
    market_units: Mapped[float] = mapped_column(Numeric(18, 4), nullable=False, default=0)
    your_units: Mapped[float] = mapped_column(Numeric(18, 4), nullable=False, default=0)
    your_units_ms: Mapped[float] = mapped_column(Numeric(18, 4), nullable=False, default=0)
    gap_sales: Mapped[float] = mapped_column(Numeric(18, 4), nullable=False, default=0)
    gap_units: Mapped[float] = mapped_column(Numeric(18, 4), nullable=False, default=0)
    raw_json: Mapped[dict] = mapped_column(JSONB, nullable=False, default=dict)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow, nullable=False)


class IqviaSnapshot(TenantBase):
    __tablename__ = 'iqvia_snapshots'
    __table_args__ = (
        Index('ix_iqvia_snapshots_imported_at', 'imported_at'),
        Index('ix_iqvia_snapshots_period', 'period_label'),
        Index('ix_iqvia_snapshots_source_sha256', 'source_sha256'),
    )
...
class IqviaLine(TenantBase):
    __tablename__ = 'iqvia_lines'
    __table_args__ = (
        Index('ix_iqvia_lines_snapshot_category', 'snapshot_id', 'category'),
        Index('ix_iqvia_lines_snapshot_manufacturer', 'snapshot_id', 'manufacturer'),
        Index('ix_iqvia_lines_snapshot_territory', 'snapshot_id', 'territory_name'),
        Index('ix_iqvia_lines_snapshot_atc3', 'snapshot_id', 'atc3'),
        Index('ix_iqvia_lines_snapshot_otc3', 'snapshot_id', 'otc3'),
        Index('ix_iqvia_lines_values', 'snapshot_id', 'values'),
    )

    id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), primary_key=True, server_default=text('gen_random_uuid()'))
    snapshot_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), ForeignKey('iqvia_snapshots.id', ondelete='CASCADE'), nullable=False)
    source_row: Mapped[int] = mapped_column(Integer, nullable=False)
    category: Mapped[str | None] = mapped_column(String(255), nullable=True)
    atc3: Mapped[str | None] = mapped_column(String(255), nullable=True)
    otc3: Mapped[str | None] = mapped_column(String(255), nullable=True)
    corporation: Mapped[str | None] = mapped_column(String(255), nullable=True)
    manufacturer: Mapped[str | None] = mapped_column(String(255), nullable=True)
    product: Mapped[str | None] = mapped_column(String(500), nullable=True)
    pack: Mapped[str | None] = mapped_column(String(500), nullable=True)
    product_label: Mapped[str | None] = mapped_column(String(1000), nullable=True)
    ...
    units: Mapped[float] = mapped_column(Numeric(18, 4), nullable=False, default=0)
    values: Mapped[float] = mapped_column(Numeric(18, 4), nullable=False, default=0)
...
class SupplierTarget(TenantBase):
    ...
    rebate_amount: Mapped[float] = mapped_column(Numeric(18, 2), nullable=False, default=0)
