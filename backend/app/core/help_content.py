...
CIRCUITS: tuple[dict[str, Any], ...] = (
    ...
    },
    {
        ...
        'steps': _tl(
            [
                ...
                'Ζήτησέ του export — π.χ. «κάνε export σε Excel τα στατιστικά πωλήσεων ανά κατηγορία» — και θα εμφανίσει κουμπί λήψης του .xlsx.',
                'Στη σελίδα Co-Pilot βλέπεις αριστερά το ιστορικό των συνομιλιών σου — κάνε κλικ για να ανοίξεις παλιά συνομιλία.',
    ...
    },
    {
        ...
        'steps': _tl(
            [
                'Διάλεξε κατάστημα και περίοδο (ηη/μμ/εεεε).',
                'KPIs + Store Health: Τζίρος, Μεικτό Κέρδος, Margin %, Έξοδα, Αξία Αποθέματος, DIO και συνολικό score υγείας.',
                'Έσοδα ανά κανάλι πώλησης και Έσοδα ανά ομάδα ειδών (καθαρή αξία + συνεισφορά %).',
                'Χαμένες πωλήσεις: ταχυκίνητα εκτός αποθέματος (χαμένος τζίρος). Βαλτωμένο απόθεμα: στοκ χωρίς κίνηση (δεσμευμένο κεφάλαιο). Και τα δύο με «Λήψη Excel».',
                'Κατηγορίες σε πτώση vs περσινή αντίστοιχη περίοδο (Δ€ / Δ%).',
                'Προτεινόμενες μεταφορές: για είδη που λείπουν εδώ & πουλάνε, προτείνει από ποιο κατάστημα να μεταφερθούν και πόσα, παίρνοντας μόνο ό,τι περισσεύει στον δότη (χωρίς να τον αδειάζει). Ο στόχος κάλυψης = η επιλεγμένη περίοδος. Excel με λεπτομέρεια (Από/Προς/Ποσότητα/Αξία). Η παραγγελία για ό,τι δεν καλύπτεται γίνεται από το FnR.',
            ],
            [
                'Pick a store and period (dd/mm/yyyy).',
                'KPIs + Store Health: revenue, gross profit, margin %, expenses, stock value, DIO and an overall health score.',
                'Revenue by sales channel and Revenue by item group (net value + contribution %).',
                'Lost sales: best-sellers out of stock (revenue at risk). Dead stock: stock with no movement (tied capital). Both with "Download Excel".',
                'Declining categories vs the same period last year (Δ€ / Δ%).',
                'Suggested transfers: for items missing here that still sell, propose which store to move them from and how many, taking only the donor\'s surplus (without draining it). Coverage target = the selected period. Detailed Excel (From/To/Qty/Value). Ordering for what transfers can\'t cover stays in FnR.',
            ],
        ),
        'columns': _tl(['Κάθε κάρτα έχει τη δική της λίστα, ταξινομημένη κατά € αξία/επίπτωση.'], ['Each card has its own list, ranked by € value/impact.']),
        'popups': _tl(['—'], ['—']),
    ...
    },
    {
        ...
        'route': '/tenant/exports/analysis',
        'group': _t('Εξαγωγές', 'Exports'),
        'shot': 'exports',
        'purpose': _t(
            'Ένα ευέλικτο εργαλείο: φτιάξε μόνος σου οποιαδήποτε αναφορά και κατέβασέ την σε Excel/CSV.',
            'One flexible tool: build any report yourself and download it to Excel/CSV.',
        ),
        'steps': _tl(
            [
                'Διάλεξε «Ομαδοποίηση κατά»: Είδος, Κανάλι πώλησης, Ομάδα, Brand, Κατηγορία 1/2/3, Υποκατάστημα ή Αποθηκευτικό χώρο.',
                'Διάλεξε «Τύπο»: Ανάλυση περιόδου ή Σύγκριση Α/Β (δύο περίοδοι — π.χ. τρέχων μήνας vs περσινός).',
                'Πρόσθεσε στήλες με «Προσθήκη στήλης» από τη λίστα πεδίων (καθαρή αξία, τεμάχια, κόστος, κέρδος, margin %, συνεισφορά %, παραστατικά, είδη, ΦΠΑ κ.ά.). Με ομαδοποίηση κατά «Είδος» ενεργοποιούνται και Barcode/Brand/Κατηγορία.',
                'Σύρε τα πλακίδια των στηλών για να αλλάξεις τη σειρά — ο πίνακας ΚΑΙ το export ακολουθούν. Το × αφαιρεί στήλη.',
                'Όρισε περίοδο (ηη/μμ/εεεε — από το ημερολόγιο ή πληκτρολογώντας) και φίλτρα (Υποκατάστημα, Αποθηκευτικός χώρος, Brands, Ομάδα, Κατηγορία 1/2/3).',
                'Η λίστα ξεκινά κενή: πάτησε «Υπολογισμός» για αποτελέσματα. «Λήψη Excel/CSV» κατεβάζει ολόκληρο το αποτέλεσμα· κάτω υπάρχει γραμμή ΣΥΝΟΛΟ. Οι επιστροφές αφαιρούνται (ίδιος κανόνας με τα dashboards).',
            ],
            [
                'Choose "Group by": Item, Sales channel, Group, Brand, Category 1/2/3, Branch or Warehouse.',
                'Choose "Mode": single-period analysis or A/B comparison (two periods, e.g. this month vs last year).',
                'Add columns with "Add column" from the field list (net value, units, cost, profit, margin %, contribution %, documents, SKUs, VAT, …). Grouping by "Item" also enables Barcode/Brand/Category.',
                'Drag the column chips to reorder — the table AND the export follow. The × removes a column.',
                'Set the period (dd/mm/yyyy — via the calendar or by typing) and filters (Branch, Warehouse, Brands, Group, Category 1/2/3).',
                'The list starts empty: press "Calculate". "Download Excel/CSV" gets the whole result; a TOTAL row is at the bottom. Returns are subtracted (same rule as the dashboards).',
            ],
        ),
        'columns': _tl(
            [
                'Οι στήλες είναι όσες διαλέξεις — και με τη σειρά που τις βάζεις.',
                'Η πρώτη στήλη είναι η διάσταση ομαδοποίησης· οι υπόλοιπες οι δείκτες που πρόσθεσες.',
            ],
            [
                'The columns are the ones you pick — in the order you arrange them.',
                'The first column is the group-by dimension; the rest are the metrics you added.',
            ],
        ),
        'popups': _tl(
