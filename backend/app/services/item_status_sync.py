...
CROSS JOIN (
  SELECT TOP 1 M.COMPANY AS COMPANY FROM MTRL M WITH (NOLOCK)
  WHERE ISNULL(M.SODTYPE, 0) = 51 GROUP BY M.COMPANY ORDER BY COUNT(*) DESC
) PRIMARYCO
LEFT JOIN MTRL PDUP WITH (NOLOCK)
  ON PDUP.CODE = I.CODE AND ISNULL(PDUP.SODTYPE, 0) = 51
 AND PDUP.COMPANY = PRIMARYCO.COMPANY AND PDUP.COMPANY <> I.COMPANY
