...
SELECT
  ...
  CAST((CASE
    WHEN ISNULL(F.SOSOURCE, 0) = 1351 AND ISNULL(F.TFPRMS, 0) = 100 AND ISNULL(F.IS_CANCELLING, 0) = 1 THEN -1
    ...
    ELSE 1
  END) * ISNULL(L.QTY1, ISNULL(L.QTY, 0)) * ISNULL(MV.stock_factor, 0) AS float) AS qty,
  CAST((CASE
    WHEN ISNULL(F.SOSOURCE, 0) = 1351 AND ISNULL(F.TFPRMS, 0) = 100 AND ISNULL(F.IS_CANCELLING, 0) = 1 THEN -1
    WHEN ISNULL(F.SOSOURCE, 0) = 1351 AND ISNULL(F.TFPRMS, 0) IN (151, 152, 181) THEN -1
    WHEN ISNULL(F.SOSOURCE, 0) <> 1351 AND ISNULL(F.TFPRMS, 0) IN (102, 181) THEN -1
  ...
  CAST((CASE
    ...
    WHEN ISNULL(F.SOSOURCE, 0) <> 1351 AND ISNULL(F.TFPRMS, 0) IN (102, 181) THEN -1
    ELSE 1
  END) * ISNULL(L.SALESCVAL, ISNULL(L.NETLINEVAL, ISNULL(L.LINEVAL, 0))) * ISNULL(MV.stock_factor, 0) AS float) AS cost_amount,
  CAST((CASE
    WHEN ISNULL(F.SOSOURCE, 0) = 1351 AND ISNULL(F.TFPRMS, 0) = 100 AND ISNULL(F.IS_CANCELLING, 0) = 1 THEN -1
    WHEN ISNULL(F.SOSOURCE, 0) = 1351 AND ISNULL(F.TFPRMS, 0) IN (151, 152, 181) THEN -1
    WHEN ISNULL(F.SOSOURCE, 0) <> 1351 AND ISNULL(F.TFPRMS, 0) IN (102, 181) THEN -1
...
FROM (
  SELECT TOP 5000
    ...
    F.CCC88EORDERNO,
    F.COMMENTS,
    ISNULL(CANC.IS_CANCELLING, 0) AS IS_CANCELLING
  FROM FINDOC F WITH (NOLOCK)
  -- A cancelling series carries the SAME behaviour code as the series it cancels
  -- (TFPRMS 100 on both the prescription-balance invoice and its cancellation), so
  -- the behaviour alone cannot give it a negative sign. SoftOne links the pair the
  -- other way round: the positive series names its cancelling one in SERIES.CSERIES,
  -- so a series that some other series points at is the cancelling half.
  -- Mirrors sales_facts.sql; SERIES is tiny, and this sits inside the already
  -- filtered TOP-5000 header subquery, so it costs next to nothing.
  OUTER APPLY (
...
-- TPRMS.FLG04): the ΤΣΥΠ repeats the prescription's quantity and cost with no stock
-- movement. Mirrors sales_facts.sql.
OUTER APPLY (
